/*
Uses: default.css, mindex.js (menu index, defines an array of the menu
*/
function writeMenu() {
  with(document) {
    for(var i=0; i<arrMenu.length; i++) {
			if(arrMenu[i].href) writeln('<div class="titel"><a href="'+arrMenu[i].href+'">'+arrMenu[i].label+'</a></div>');
      else writeln('<div class="titel">'+arrMenu[i].label+'</div>');
      for(var j=0; j<arrMenu[i].items.length; j++) {
        var href=arrMenu[i].items[j].href; 
        var hreflang=arrMenu[i].items[j].hreflang;
        if(hreflang) hreflang=' hreflang="'+hreflang+'"'; else hreflang='';
        var label=arrMenu[i].items[j].label;
        var pn=location.pathname;
        if(pn.substr(pn.length-1)=='/') pn+='index.htm'; //default document
        var blnSelected=pn.indexOf('/'+fnPart(href))>=0&&href.length>0;
        if(href.indexOf('?run=')>=0) {
          var page=href.substring(0,href.indexOf('?run='));
          if(pn.lastIndexOf(page)==pn.length-page.length||(pn.lastIndexOf('/')==pn.length-1 && page=='index.htm')) {
            //set javascript, when href is current page (including exception '/' is default document '/index.htm')
            var k=href.lastIndexOf('?run=')+'?run='.length;
            href='javascript: void '+unescape(href.substr(k));
          }
          else {
            //set href (file?run=ding)
          }
        }
        if(blnSelected) write('<div class="selected item">');
        else  write('<div class="item">');
        if(href.length>0) {
          writeln('<a href="'+href+'"'+hreflang+(arrMenu[i].items[j].title==null?'':' title="'+arrMenu[i].items[j].title+'"')+'>'+label+'</a></div>');
        }
        else {
          writeln('<span style="color: gray">'+label+'</span></div>');
        }
      }
      writeln('<div class="item">&nbsp;</div>');
    }
  }
  startUrchin();
}

function fnPart(href) {
  //strips the bookmark part and/or querystring part 
  var i=href.indexOf('#');
  if(i==-1) i=href.indexOf('?');
  return href.substr(0,i==-1?href.length:i);
}

function startUrchin() {
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-321198-3', 'auto');
  ga('send', 'pageview');
}

/*
//--| Op deze manier, want writeln("</script>") geeft problemen met IE
function startUrchin() {
  _uacct = "UA-321198-3";
  urchinTracker();
}
