var aHTMLEntities=
[ { sTitle:'Character entity references for ISO 8859-1 characters'
  , aList:
...
, { sTitle:'Character entity references for symbols, mathematical symbols, and Greek letters'
  , aList:
...
, { sTitle:'Character entity references for markup-significant and internationalization characters'
  , aList:
...
function process(s)
{
	var re=/<!ENTITY ([^ ]+) *CDATA "([^"]+)" +-- ([^>]*)/i;
	var m='<!ENTITY';
	var i,j=0;
	var res=[];
	while((i=s.indexOf(m,j))>=0)
	{
		var a=s.substr(i).match(re);
		if(a!=null&&a.length==4)
		{
			res[res.length]='["'+a[1]+'","'+a[2]+'","'+a[3].replace(/[\r\n]/g,' ').replace(/[ ][ ]+/g,' ').replace(/[ ]*--$/,'').replace(/"/g,'\\\"')+'"]';
		}
		j=i+1;
	}
	return '[ '+res.join('\n, ')+'\n];';
