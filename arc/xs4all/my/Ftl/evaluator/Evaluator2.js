...
function EvaluatorCallback()
{
  console.info("scheduling callback");
  window.setTimeout('evaluator.doSingleCalc(true);',25);
}
...
Evaluator.prototype.onDone=function(fnPointer)
{
  this.onDoneHandler=fnPointer; 
}
...
Evaluator.prototype.prepare=function()
{
    ...
    {
      var item=this.masterDependancyGraph[v];
      for (var i=0;i<item.depList.length ;i++ )
      {
        if (this.masterDependancyGraph[item.depList[i]])
        {
          this.masterDependancyGraph[item.depList[i]].inDegree++ ;  
        }
      }
    }
}
...
Evaluator.prototype.calc=function(arrDirtyNodes,blnAsync)
{
  ...
  if (arrDirtyNodes==null)
  {
     this.calcGraph=this.masterDependancyGraph;
     this.doCalc(blnAsync);
  }
  else
  {
     this.calcGraph=this.getDependancySubGraph(arrDirtyNodes);
     this.doCalc(blnAsync); 
  }
}
  ...
  for (var i=0;i<arrDirtyNodes.length ;i++ )
  {
    ...
    while (arrStack.length>0)
    {
      ...
      if (!this.masterDependancyGraph[obj.right].visited)
      {
        ...
        this.masterDependancyGraph[obj.right].visited=true;
      }
...
Evaluator.prototype.doSingleCalc=function(blnAsync)
{
    ...
    if (!allDone)
    {
      ...
      // the reason that this happens first is that async calls "never" come back, but rather restart the whole logic when they are finished.
      ...
      if (blnAsync)
      {
        this.fireCalcEvent(cur,EvaluatorCallback);
      }  
      else
      {
