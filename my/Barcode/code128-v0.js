new function() {
  ...
  function code128(code, barcodeType) {
    var a = parseBarcode(code, arguments.length==2 ? barcodeType : code128Detect(code));
  ...
  function parseBarcode(barcode, barcodeType) {
    ...
    bars.add = function(nr) {
      var nrCode = BARS[nr];
      this.check = this.length==0 ? nrCode : this.check + nrCode*this.length;
  ...
  var fromType = {
    A: function(charCode) {
      if (charCode>=0 && charCode<64) return charCode+32;
      if (charCode>=64 && charCode<96) return charCode-64;
      return charCode;
    },
    B: function(charCode) {
  ...
  //--| CSS (code 128.css) as javascript object
